...
App.AlertDefinition = DS.Model.extend({
  ...
  labelDisplayName : function () {
    var labelName = this.get('service.name');
    if (!labelName) {
      labelName = this.get('label');

      if (labelName) {
        labelName=labelName.replace('Ambari', 'Throld');
        labelName = labelName.toCapital();
      }
    }
    return labelName;
  }.property('labelName', 'service.name'),
  /**
